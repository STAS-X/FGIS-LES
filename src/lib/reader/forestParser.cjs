...
const lotAdditionalData = [
    ...
    {
        value: 'СОСТАВ |ПОЛНОТА |ТЛУ ВАРЬИРУЕТ|ТИП ЛЕСА |ТРЕБУЕТСЯ ПРОВЕДЕНИЕ |ДОСТУП ОГРАНИЧЕН|ИСП.БЕЗ РАЗРЕШИТЕЛЬНЫХ ДОК.|НАСАЖДЕНИЕ |НА ПЛОЩАДИ |РАЗМЕЩЕНИЕ |ВЫСОТА |ДИАМЕТР |ТИП ЛЕСА |ПЛОЩАДЬ |БОНИТЕТ |РЕКОМЕНД.|ЗАЩ.ПОЛ.ЛЕСА ВДОЛЬ |УЧ-КИ ЛЕСА ВОКРУГ |УЧ-КИ ЛЕСА ДО |УЧ-КИ ЛЕСА ВОКР НАСЕЛ.|УЧ.ЛЕСА ВОКР.|ЧАСТИЧНО |РЕКОМ.К|РЕКОМЕНДАЦИИ|ПРОВЕДЕНО СОД|ПЛС.ВДОЛЬ |ПОЛОСЫ ПО БЕР.РЕК |ПОЛОСЫ ВДОЛЬ РЕК|В ВЫДЕЛЕ |ЗАГРЯЗНЕНИЕ БЫТОВЫМИ ',
        name: 'особенности ',
        isElement: false,
    },
...
const lotActivities = [
    ...
    { mask: 'ус.*во.*мест.*от.*|уст.*ка.*лес.*ме.*', code: 2371 },
    { mask: 'комб.*лесов.*', code: 3221 },
    {
        mask: 'агротех.*уход.*',
        hasRtk: true,
        code: 3261,
...
const lotProtectZoneOzu = [
    ...
    {
        name: 'ЛЕСОСЕМ.*ПЛАНТАЦ.*|ОБЪЕКТ.*ЛЕС.*СЕМЕНОВОДСТВА|ПОСТОЯН.*ЛЕСОСЕМ.*УЧ.*КИ',
        code: 323,
    },
    {
...
const lotExtraLandType = [
    {
        value: 'фонд.*руб.*|длит.*фонд.*пост.*руб.*',
        short: 'Фонд рубок',
        type: 'лесные земли',
        isPreview: true,
        code: '1509',
...
class ForestParser {
    ...
    #parseKvartalContent = (textContent) => {
        ...
        if (textContent.search(/Квартал /i) > -1) {
            this.#currentStep = searchStep.stepFeature;
            this.#currentStep = searchStep.stepFeature;
            // Добавляем фильтрацию по числовым символам через регулярку для исключения "нечисловых" номеров кварталов - 35 книга в Ивановскм лесничестве номера 1Л - 7Л
            newKvartal = Number(
                textContent
                    .slice(textContent.search(/Квартал /i) + 8)
                    .match(/\d+/)
            );
